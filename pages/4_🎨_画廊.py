...
# 创建一个列表，每个元素是一个元组，包含标签的名字和图片的地址
# tabs = [("Cat", "https://static.streamlit.io/examples/cat.jpg"),
#         ("Dog", "https://static.streamlit.io/examples/dog.jpg"),
#         ("Owl", "https://static.streamlit.io/examples/owl.jpg"),
#         # ... 其他97个元素
#        ]
#
...
st.markdown("如果你对详细信息感兴趣，就去检索吧:mag: ")
...
dashown = da.sample(20)

# print(type(dashown))
#
# print(dashown[1].chunks)
#
# dashown[1].chunks.summary()
...
for row in rows:
   with row[0]:
      st.text(dashown[:, 'main_text'][count].text)
      st.image(dashown[:, 'image'][count].uri)
      count += 1
   with row[1]:
      st.text(dashown[:, 'main_text'][count].text)
      st.image(dashown[:, 'image'][count].uri)
